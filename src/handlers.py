...
from utils.functions import split_message, make_grid
...
async def process_framework(message: Message, state: FSMContext) -> None:
    ...
    kb = make_grid(4, [KeyboardButton(text=title.title) for title in titles])
    print(kb)
    keyboard = ReplyKeyboardMarkup(
        keyboard=kb,
        resize_keyboard=True,
...
async def give_final_parse(message: Message, state: FSMContext) -> None:
    ...
    current_link = links.get(message.text, None)
    ...
    for part in text_parts:
        await message.reply(part)
    await message.reply(f"Хотите ещё почитать про {framework}?")
